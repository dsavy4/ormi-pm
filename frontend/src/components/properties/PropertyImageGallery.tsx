...
import { Eye, Trash2, ImageIcon, Star, Crown } from 'lucide-react';
...
interface PropertyImageGalleryProps {
  ...
  onCoverChange?: (newCoverImage: string) => void; // New callback for cover changes
  canDelete?: boolean;
  className?: string;
}
...
export const PropertyImageGallery: React.FC<PropertyImageGalleryProps> = ({
  ...
  const getImageFileName = (imageUrl: string) => {
    try {
      ...
      return pathParts[pathParts.length - 1] || 'property-image';
    } catch {
  ...
  const setAsCover = async (imageUrl: string) => {
    ...
    // Notify parent about cover change for listing refresh
    if (onCoverChange) {
  ...
  const handleDeleteImage = async (imageUrl: string) => {
    ...
    setIsDeleting(true);
    try {
      const response = await propertiesApi.deletePropertyImage(propertyId, imageUrl) as any;
      ...
      if (response.success) {
        ...
        toast({
          title: "Image deleted",
          description: "Property image has been removed successfully.",
        });
      } else {
        toast({
          title: "Delete failed",
          description: response.error || "Failed to delete image. Please try again.",
          variant: "destructive",
        });
      }
    } catch (error) {
      ...
      <Card className={cn('w-full', className)}>
        <CardContent className="p-6">
          <div className="flex flex-col items-center justify-center py-12 text-center">
            ...
            <p className="text-sm text-muted-foreground">Upload some photos to showcase this property</p>
          </div>
        </CardContent>
      </Card>
    );
